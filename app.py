# app.py - VibeNet  (SQLAlchemy ORM  |  pg8000  |  Supabase PostgreSQL  |  Supabase Storage  |  Supabase Auth)
import os
import uuid
import datetime
...
class User(db.Model):
    ...
    password          = db.Column(db.Text, nullable=True)  # Supabase Auth handles passwords
    supabase_uid      = db.Column(db.Text, unique=True, nullable=True)  # Supabase Auth UID
...
const _sbAuth  = window.supabase ? window.supabase.createClient(_SB_URL, _SB_KEY) : null;

// Listen for auth state changes (handles token refresh, tab restore)
if(_sbAuth) {
  _sbAuth.auth.onAuthStateChange(async (event, sbSession) => {
    if(event === 'SIGNED_IN' && sbSession && !currentUser) {
      await _syncVibeNetUser(sbSession);
    }
    if(event === 'SIGNED_OUT') {
      currentUser = null;
...
async function _syncVibeNetUser(sbSession) {
  const { user } = sbSession;
...
async function signup(){
  ...
  if(!_sbAuth){ alert('Auth not initialized.'); return; }

  // 1. Sign up with Supabase Auth
  const { data: sbData, error: sbErr } = await _sbAuth.auth.signUp({ email, password });
  if(sbErr){ alert(sbErr.message); return; }
  ...
  // 2. Check if email confirmation required
  if(!sbSession) {
    alert('📧 Check your email and confirm your account, then log in.');
    return;
  }

  // 3. Create VibeNet profile
...
async function login(){
  ...
  const { data: sbData, error: sbErr } = await _sbAuth.auth.signInWithPassword({ email, password });
  if(sbErr){ alert(sbErr.message); return; }

  // 2. Sync with VibeNet backend
  await _syncVibeNetUser(sbData.session);
}
...
def _verify_supabase_token(token):
    ...
    try:
        ...
        print(f"Supabase token verify error: {e}")
    return None


@app.route("/api/signup", methods=["POST"])
def api_signup():
    """Called after Supabase Auth signup — creates VibeNet user profile."""
    ...
    # Verify token
    sb_user = _verify_supabase_token(access_token)
    if not sb_user:
    ...
    user = User(name=name, email=email, password=None,
                supabase_uid=supabase_uid, profile_pic=profile_pic)
    db.session.add(user)
    db.session.commit()
    session["user_email"] = email
...
def api_login():
    """Called after Supabase Auth login — returns VibeNet user profile."""
    data         = request.get_json() or {}
    email        = data.get("email", "").strip().lower()
    supabase_uid = data.get("supabase_uid", "").strip()
    access_token = data.get("access_token", "")
    ...
    if not user:
        ...
        name = email.split("@")[0]
        user = User(name=name, email=email, password=None,
                    supabase_uid=supabase_uid, profile_pic="")
        db.session.add(user)
        db.session.commit()
