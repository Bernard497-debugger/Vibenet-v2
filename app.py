...
def verify_clerk_token(token):
    """Verify Clerk session token and return user info or None."""
    ...
    try:
        ...
        if resp.status_code == 200:
            return resp.json()
        # Try JWT verification via Clerk's verify endpoint
        resp2 = requests.post(
        ...
        print(f"Clerk verify error: {e}")
    return None

def get_clerk_user(token):
    """Get Clerk user details from token."""
    if not CLERK_SECRET_KEY or not token:
        return None
    try:
        # Decode JWT to get user_id (without verification for speed, verify separately)
        import base64
        parts = token.split(".")
        if len(parts) >= 2:
            payload = parts[1] + "=="  # pad
        ...
        print(f"Clerk get_user error: {e}")
    return None
...
class User(db.Model):
    ...
    password          = db.Column(db.Text, nullable=True)   # nullable: Clerk handles auth
...
<script>
  // Clerk publishable key injected from server
  window.CLERK_PUBLISHABLE_KEY = "{{ clerk_pk }}";
</script>
    ...
    try {
      ...
      if(verified.status === 'complete') {
        ...
        const res = await fetch(API + '/signup', {
          ...
          body: JSON.stringify({ name, email, clerk_token: token, profile_pic: profilePicUrl })
        });
        const j = await res.json();
      ...
      console.warn('Clerk signup failed, falling back:', e.message);
    }
  }

  // --- Fallback: legacy signup ---
  let profilePicUrl = '';
  const pic = byId('signupPic').files[0];
  if(pic){
    try { profilePicUrl = await uploadFile(pic, 'vibenet/avatars'); }
    ...
    try {
      ...
      if(signIn.status === 'complete') {
        ...
        const res = await fetch(API + '/login', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({ email, clerk_token: token })
        });
        const j = await res.json();
        if(j.user){ currentUser = j.user; onLogin(); } else alert(j.error || 'Login failed');
        return;
      }
    } catch(e) {
      console.warn('Clerk login failed, falling back:', e.message);
    }
  }
...
@app.route("/")
def index():
    return render_template_string(HTML, clerk_pk=CLERK_PUBLISHABLE_KEY)
...
def api_signup():
    """Create or fetch user after Clerk authentication."""
    ...
    if clerk_token and CLERK_SECRET_KEY:
        clerk_user = get_clerk_user(clerk_token)
        if not clerk_user:
            return jsonify({"error": "Invalid Clerk token"}), 401
        # Use Clerk's verified email
        clerk_emails = clerk_user.get("email_addresses", [])
        if clerk_emails:
            email = clerk_emails[0].get("email_address", email).lower()
        if not name:
            first = clerk_user.get("first_name", "") or ""
            last  = clerk_user.get("last_name", "") or ""
            name  = (first + " " + last).strip() or email.split("@")[0]
        if not profile_pic:
            profile_pic = clerk_user.get("image_url", "")
    ...
    user = User(name=name, email=email, password=None, profile_pic=profile_pic)
    db.session.add(user)
    db.session.commit()
    session["user_email"] = email
...
def api_login():
    """Login via Clerk token or legacy email/password."""
    data        = request.get_json() or {}
    email       = data.get("email", "").strip().lower()
    password    = data.get("password", "")
    clerk_token = data.get("clerk_token", "")

    # --- Clerk token login (primary) ---
    if clerk_token and CLERK_SECRET_KEY:
        clerk_user = get_clerk_user(clerk_token)
        if not clerk_user:
            return jsonify({"error": "Invalid Clerk token"}), 401
        ...
        if not user:
            ...
            last  = clerk_user.get("last_name", "") or ""
            name  = (first + " " + last).strip() or email.split("@")[0]
            pic   = clerk_user.get("image_url", "")
            user  = User(name=name, email=email, password=None, profile_pic=pic)
            db.session.add(user)
            db.session.commit()
        session["user_email"] = email
        return jsonify({"user": user.to_dict()})

    # --- Legacy email/password login (fallback for existing users) ---
