...
class User(db.Model):
    ...
    supabase_uid      = db.Column(db.Text, unique=True, nullable=True)
...
.auth-btn-loading { opacity: 0.85; pointer-events: none; }
</style>
...
async function signup(){
  ...
  const txt = byId('signupBtnText');
  const spin = byId('signupSpinner');
  ...
  try {
    ...
    const j = await res.json();
    if(j.user){ currentUser = j.user; done(); onLogin(); }
    else done(j.error || j.message);
  } catch(e) {
    done('Signup failed: ' + e.message);
  }
}
...
async function login(){
  ...
  const txt = byId('loginBtnText');
  const spin = byId('loginSpinner');
  btn.classList.add('auth-btn-loading');
  ...
  try {
    ...
    const j = await res.json();
    if(j.user){ currentUser = j.user; done(); onLogin(); }
    else done(j.error || 'Invalid credentials');
  } catch(e) {
    done('Login failed: ' + e.message);
  }
}
