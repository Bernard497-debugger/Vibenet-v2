  ...
  if(fileEl.files[0]){
    ...
    try {
      ...
      if(isImage){
        ...
        // Videos: Direct upload (no FFmpeg compression — too slow on mobile)
        console.log('🎬 Video - direct upload');
        showUploadProgress(true, 'Uploading video...');
        
        // Try to extract thumbnail first
        let thumbnailBlob = null;
        try {
          thumbnailBlob = await extractVideoThumbnail(file);
        } catch(e) {
        ...
        try {
          ...
          if(j.url){
            ...
            console.warn('⚠️ Video upload failed:', j.error);
          }
        } catch(e) {
          showUploadProgress(false);
