...
# SQLAlchemy: Supabase PostgreSQL via pg8000 (pure Python - no compiled binary needed)
_raw_db_url = os.environ.get("SUPABASE_DB_URL", "")
if not _raw_db_url:
    raise RuntimeError("SUPABASE_DB_URL is not set. Add it in Render dashboard -> Environment.")

if _raw_db_url.startswith("postgresql://"):
    DATABASE_URL = _raw_db_url.replace("postgresql://", "postgresql+pg8000://", 1)
elif _raw_db_url.startswith("postgres://"):
    DATABASE_URL = _raw_db_url.replace("postgres://", "postgresql+pg8000://", 1)
else:
  ...
  <div class="auth-wrap" style="grid-template-columns:1fr;max-width:440px;border-radius:24px">
    ...
    <div class="auth-forms" style="padding:0 24px 32px">
      ...
      <div id="authSignup">
        ...
        <button id="signupBtn" class="btn-primary" onclick="signup()" style="width:100%;margin-top:4px;display:flex;align-items:center;justify-content:center;gap:10px;">
          <span id="signupBtnText">Create Account →</span>
          <span id="signupSpinner" style="display:none;width:18px;height:18px;border:2px solid rgba(6,9,16,0.3);border-top-color:#060910;border-radius:50%;animation:spin 0.7s linear infinite"></span>
        </button>
      ...
      <div id="authLogin" style="display:none">
        ...
        <button id="loginBtn" class="btn-primary" onclick="login()" style="width:100%;display:flex;align-items:center;justify-content:center;gap:10px;">
          <span id="loginBtnText">Sign In →</span>
          <span id="loginSpinner" style="display:none;width:18px;height:18px;border:2px solid rgba(6,9,16,0.3);border-top-color:#060910;border-radius:50%;animation:spin 0.7s linear infinite"></span>
        </button>
        <div style="text-align:center;margin-top:10px">
          <span onclick="openForgotPassword()" style="font-size:12px;color:var(--accent);cursor:pointer;font-weight:500">Forgot password?</span>
        </div>
        <div style="text-align:center;margin-top:8px;font-size:12px;color:#5a6a85">Don't have an account? <span onclick="switchAuthTab('signup')" style="color:var(--accent);cursor:pointer;font-weight:600">Create one</span></div>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="app-layout">
    ...
    <div class="main-col">
      ...
      <div id="profile" class="tab">
        ...
        <!-- Settings Section -->
        <div class="monet-section-title" style="margin-top:24px">⚙️ Settings</div>
        <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;overflow:hidden;margin-bottom:16px">
          <button onclick="openChangePassword()" style="width:100%;padding:16px 18px;background:transparent;border:none;border-bottom:1px solid var(--border);color:var(--text);font-size:14px;font-family:'DM Sans',sans-serif;cursor:pointer;text-align:left;display:flex;align-items:center;justify-content:space-between;transition:background 0.2s" onmouseover="this.style.background='rgba(255,255,255,0.03)'" onmouseout="this.style.background='transparent'">
            <span style="display:flex;align-items:center;gap:10px"><span style="font-size:16px">🔑</span> Change Password</span>
            <span style="color:var(--muted);font-size:12px">→</span>
          </button>
          <button onclick="logout()" style="width:100%;padding:16px 18px;background:transparent;border:none;color:#f06a4d;font-size:14px;font-family:'DM Sans',sans-serif;cursor:pointer;text-align:left;display:flex;align-items:center;gap:10px;transition:background 0.2s" onmouseover="this.style.background='rgba(240,106,77,0.05)'" onmouseout="this.style.background='transparent'">
...
<div id="forgotModal" class="modal-overlay" style="display:none" onclick="if(event.target===this)closeForgotPassword()">
  <div class="modal-box" style="max-width:400px">
    <div class="modal-title">🔑 Forgot Password</div>
    <div style="font-size:13px;color:#8899b4;margin-bottom:16px">Enter your email and we'll send you a password reset link.</div>
    <div class="field">
      <div class="field-label">Email</div>
      <input id="forgotEmail" type="email" placeholder="you@email.com" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:11px 14px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:14px;outline:none" />
    </div>
    <div id="forgotMsg" style="display:none;font-size:13px;margin-top:10px;line-height:1.6"></div>
    <div class="modal-footer">
      <button class="btn-ghost" onclick="closeForgotPassword()">Cancel</button>
      <button id="forgotBtn" class="btn-primary" onclick="sendForgotPassword()" style="display:flex;align-items:center;gap:8px">
        <span id="forgotBtnText">Send Reset Link</span>
        <span id="forgotSpinner" style="display:none;width:16px;height:16px;border:2px solid rgba(6,9,16,0.3);border-top-color:#060910;border-radius:50%;animation:spin 0.7s linear infinite"></span>
      </button>
    </div>
  </div>
</div>
...
<div id="changePwModal" class="modal-overlay" style="display:none" onclick="if(event.target===this)closeChangePassword()">
  <div class="modal-box" style="max-width:400px">
    ...
    <div style="font-size:13px;color:#8899b4;margin-bottom:16px">A password reset link will be sent to <strong id="changePwEmail" style="color:var(--accent)"></strong></div>
    <div id="changePwMsg" style="display:none;font-size:13px;margin-top:10px;line-height:1.6"></div>
    <div class="modal-footer">
      <button class="btn-ghost" onclick="closeChangePassword()">Cancel</button>
      <button id="changePwBtn" class="btn-primary" onclick="sendChangePassword()" style="display:flex;align-items:center;gap:8px">
        <span id="changePwBtnText">Send Reset Link</span>
        <span id="changePwSpinner" style="display:none;width:16px;height:16px;border:2px solid rgba(6,9,16,0.3);border-top-color:#060910;border-radius:50%;animation:spin 0.7s linear infinite"></span>
      </button>
    </div>
  </div>
</div>
  ...
  if(fileEl.files[0]){
    ...
    try {
      ...
      if(isImage){
        ...
        // Videos: Use uploadFile for compression
        console.log('🎬 Video - compression + upload');
...
async function changeProfilePic(input){
  ...
  try {
    ...
    if(cfg.url && cfg.key && cfg.bucket) {
      // Upload directly from browser to Supabase Storage
      const fileId = crypto.randomUUID().replace(/-/g,'');
      const filePath = 'avatars/' + fileId + '.jpg';
      const uploadUrl = cfg.url + '/storage/v1/object/' + cfg.bucket + '/' + filePath;
      ...
      if(uploadRes.ok || uploadRes.status === 200 || uploadRes.status === 201) {
        const publicUrl = cfg.url + '/storage/v1/object/public/' + cfg.bucket + '/' + filePath;
        // Save URL to DB via backend
        const saveRes = await fetch(API + '/update_profile_pic_url', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({email: currentUser.email, profile_pic: publicUrl})
        });
        const j = await saveRes.json();
        if(j.success){
          currentUser.profile_pic = publicUrl;
          byId('profileAvatar').src = publicUrl;
          byId('composerAvatar').src = publicUrl;
        } else {
...
async function loadMonetization(){
  ...
  if(payoutSection){
    payoutSection.style.display = 'block';  // Always show
    
    // Disable inputs if not eligible
    const payoutNumber = byId('payoutNumber');
    const payoutAmount = byId('payoutAmount');
    const payoutBtn = payoutSection.querySelector('button');
    
    if(!eligible){
      if(payoutNumber) payoutNumber.disabled = true;
      if(payoutAmount) payoutAmount.disabled = true;
      if(payoutBtn) {
      ...
      if(payoutNumber) payoutNumber.disabled = false;
      if(payoutAmount) payoutAmount.disabled = false;
      if(payoutBtn) {
...
function openForgotPassword(){
  byId('forgotEmail').value = byId('loginEmail') ? byId('loginEmail').value : '';
  byId('forgotMsg').style.display = 'none';
  byId('forgotModal').style.display = 'flex';
  setTimeout(()=>byId('forgotEmail').focus(), 80);
}
function closeForgotPassword(){ byId('forgotModal').style.display = 'none'; }

async function sendForgotPassword(){
  const email = byId('forgotEmail').value.trim().toLowerCase();
  if(!email){ alert('Please enter your email.'); return; }

  const btn   = byId('forgotBtn');
  const txt   = byId('forgotBtnText');
  const spin  = byId('forgotSpinner');
  ...
  try {
    ...
    if(j.success){
      msg.style.color = 'var(--accent)';
      msg.textContent = '✅ Reset link sent! Check your email (' + email + ') and click the link to set a new password.';
      btn.style.display = 'none';
    } else {
      msg.style.color = 'var(--danger)';
      msg.textContent = '❌ ' + (j.error || 'Something went wrong.');
      btn.disabled = false; txt.textContent = 'Send Reset Link'; spin.style.display = 'none';
    }
  } catch(e) {
    msg.style.display = 'block';
    msg.style.color = 'var(--danger)';
...
function openChangePassword(){
  if(!currentUser) return;
  byId('changePwEmail').textContent = currentUser.email;
  byId('changePwMsg').style.display = 'none';
  byId('changePwBtn').style.display = 'flex';
  byId('changePwBtnText').textContent = 'Send Reset Link';
  byId('changePwSpinner').style.display = 'none';
  byId('changePwBtn').disabled = false;
  byId('changePwModal').style.display = 'flex';
}
function closeChangePassword(){ byId('changePwModal').style.display = 'none'; }

async function sendChangePassword(){
  if(!currentUser) return;
  const btn  = byId('changePwBtn');
  const txt  = byId('changePwBtnText');
  const spin = byId('changePwSpinner');
  const msg  = byId('changePwMsg');
  btn.disabled = true; txt.textContent = 'Sending...'; spin.style.display = 'inline-block';
  ...
  try {
    ...
    if(j.success){
      msg.style.color = 'var(--accent)';
      msg.textContent = '✅ Password reset link sent to ' + currentUser.email + '. Check your inbox!';
      btn.style.display = 'none';
    } else {
      msg.style.color = 'var(--danger)';
      msg.textContent = '❌ ' + (j.error || 'Something went wrong.');
...
def api_forgot_password():
    ...
    try:
        redirect_url = os.environ.get("SITE_URL", "https://vibenet-v2.onrender.com")
        resp = requests.post(
        ...
        return jsonify({"error": f"Auth error ({resp.status_code})"}), 500
    except Exception as e:
...
# ---------- Supabase config for client-side uploads ----------
...
@app.route("/api/update_profile_pic_url", methods=["POST"])
def api_update_profile_pic_url():
    data = request.get_json() or {}
    email = data.get("email", "").strip()
    pic_url = data.get("profile_pic", "").strip()
    if not email or not pic_url:
